...
if __name__ == "__main__":
    ...
    net = PixelNeRFNet().cuda()
    net.load_state_dict(torch.load('/home/liqi/code/AutoRF-pytorch/output/200.ckpt'))
    net = net.eval()

    dataset = KITTI('train.txt')
    image, all_rays, rois, intersect, objs, calib = dataset.__getscene__(10, distinct_intersect=True, get_calib=True)

    src_images = rois.cuda()
    ...
    with torch.no_grad():
        ...
        for i in range(0, B, args.chunk):
            xyz = xyz_[None, i:i+args.chunk]
            d = dir_[None, i:i+args.chunk]
