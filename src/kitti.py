...
def rotate(objs, angle):
    #  objs[:, :3] = kitti_util.rotate_yaw(objs[:, :3], angle)
    objs[:, 6] += angle
...
class KITTI(torch.utils.data.Dataset):
    ...
    def __init__(self, split_file):
        ...
        self.split_file = f"{DATA_DIR}/ImageSets/{split_file}"

        with open(self.split_file, "r") as f:
            selected_idx = [line.rstrip('\n') for line in f.readlines()]

        tmp = [f[:-10] for f in os.listdir(f"{DATA_DIR}/training/{LABEL_DIR}") if "label" in f ]
        tmp.sort()

        self.filelist = []
        for file in tmp:
            if file[:-3] in selected_idx:
    ...
    def __getitem__(self, idx):
        ...
        img = cv2.imread(f'{DATA_DIR}/training/{LABEL_DIR}/%s_patch.png' % id)
        msk = cv2.imread(f'{DATA_DIR}/training/{LABEL_DIR}/%s_mask.png' % id)

        with open(f'{DATA_DIR}/training/{LABEL_DIR}/%s_label.txt' % id , 'r') as f:
            obj = f.readlines()[0].split()

        sid = id[:6]


        calib = kitti_util.Calibration(f'{DATA_DIR}/training/calib/%s.txt' % sid)
        imshape = cv2.imread(f'{DATA_DIR}/training/image_2/%s.png' % sid).shape
    ...
    def __getviews__(self, idx,
        ...
        calib = kitti_util.Calibration(f'{DATA_DIR}/training/calib/%s.txt' % sid)
        canvas = cv2.imread(f'{DATA_DIR}/training/image_2/%s.png' % sid)
    ...
    def __getscene__(self, sid, translation=None, rotation=None, distinct_intersect=False, get_calib=False,):
        calib = kitti_util.Calibration(f'{DATA_DIR}/training/calib/%06d.txt' % sid)
        canvas = cv2.imread(f'{DATA_DIR}/training/image_2/%06d.png' % sid)
        ...
        objs = kitti_util.read_label(f'{DATA_DIR}/training/label_2/%06d.txt' % sid)
