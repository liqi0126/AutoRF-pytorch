...
class KITTI(torch.utils.data.Dataset):
    ...
    def __init__(self, ):
        ...
        self.filelist = [f[:-10] for f in os.listdir(f"{DATA_DIR}/nerf") if "label" in f ]
        self.filelist.sort()
    ...
    def __getitem__(self, idx):
        ...
        img = cv2.imread(f'{DATA_DIR}/nerf/%s_patch.png' % id)
        msk = cv2.imread(f'{DATA_DIR}/nerf/%s_mask.png' % id)

        with open(f'{DATA_DIR}/nerf/%s_label.txt' % id , 'r') as f:
            obj = f.readlines()[0].split()

        sid = id[:6]


        calib = kitti_util.Calibration(f'{DATA_DIR}/calib/%s.txt' % sid)
        imshape = cv2.imread(f'{DATA_DIR}/image_2/%s.png' % sid).shape
    ...
    def __getviews__(self, idx,
        ...
        calib = kitti_util.Calibration(f'{DATA_DIR}/calib/%s.txt' % sid)
        canvas = cv2.imread(f'{DATA_DIR}/image_2/%s.png' % sid)
